...
.text
	...
	promptLoop:
		...
		read_n(t0)
	checkPrompt:
		bgtz t0, initializeVars		# input > 0
		print_str(invalidMsg)		# if not, invalid message and jump to loop
		print_str(newline)
		j promptLoop
	initializeVars:
		mv t1, t0					# outer loop counter
		li t3, 0					# counter for the spaces
		li t4, 0					# length of file
		
	layerLoop:
		li t2, 0 					# inner loop counter
		add t5, t1, t1 				# num of stars
		addi t5, t5, -1
	spaceLoop:
		beqz t3, spaceCheck
		print_str(blankspace)
		write_to_buffer(0x20)		# writing " " to file
	spaceCheck:
		addi t3, t3, -1				# decrementing by 1
		bgtz t3, spaceLoop
	starLoop:
		print_str(star)
		write_to_buffer(0x2a)		# writing * to file
		addi t4, t4, 1				# adding 1 to length of file
	starCheck:
		addi t2, t2, 1				# incrementing layer by 1
		blt t2, t5, starLoop		# goes back to stars if there aren't enough stars in the layer
	layerCheck:
		addi t1, t1, -1				# decrementing by 1 
		sub t3, t0, t1				# setting space counter to current layer 
		print_str(newline)
